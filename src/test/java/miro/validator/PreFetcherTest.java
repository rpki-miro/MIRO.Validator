...
import main.java.miro.validator.PreFetcher;
...
public class PreFetcherTest {
	...
	public void testReadWritePrefetchURIs() {
		PreFetcher preFetcher = new PreFetcher("src/test/resources/prefetchURIs");

		URI u1 = URI.create("rsync://rpki.example.org");
		URI u2 = URI.create("rsync://rpki.example.com");
		preFetcher.addURI(u1);
		preFetcher.addURI(u2);
		preFetcher.writePrefetchURIsToFile();

		preFetcher = new PreFetcher("src/test/resources/prefetchURIs");
		assertTrue(preFetcher.getPrefetchURIs().contains(URI.create("rsync://rpki.example.com")));
		assertTrue(preFetcher.getPrefetchURIs().contains(URI.create("rsync://rpki.example.org")));
	}
